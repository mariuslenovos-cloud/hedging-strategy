...
const string StatsCSVFile        = "gridstat_setups_gold.csv";  // per-symbol: e.g. gridstat_setups_oil.csv  // LOCKED (was input)
...
const double MinWinRate            = 0.55;  // LOCKED (was input)
...
const bool UseRiskNormalizedLots = false;   // OFF for gold; ON for volatile symbols (silver/oil)  // LOCKED (was input)
const double RiskPctPerTrade       = 3.0;  // LOCKED (was input)
...
const int MaxConcurrentBaskets  = 1;    // 1 = one cluster at a time (gold/silver: UNCHANGED path). >1 = allow N independent baskets so a stuck basket can't starve 200+ signals (trending indices/oil). No time/hard stops.  // LOCKED (was input)
...
const int RescueMaxBaskets      = 1;        // rescue slots beyond MaxConcurrentBaskets (fib C ramp: 2-3)  // LOCKED (was input)
