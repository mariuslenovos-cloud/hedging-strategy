...
#define EA_BUILD_VERSION "MT5-2026-06-20-S17"
...
void OnTick()
{
   ...
   if(UseDailyTrendFilter)
   {
      ...
      if(dir==ORDER_TYPE_BUY && !bull) return;
      if(dir==ORDER_TYPE_SELL && bull) return;
   }
   if(!IsTrendStrong()) return;

   // capture the discarded features at decision time (same shifts the filters use)
   double adx     = ReadBuf(hADX,0,0);
   ...
   if(StatsCollectionMode) { AddPendingSignal(key, dir, bigMove, wpr, adx, maAngle, runup, periodUsed, barsInBand); return; }

   // ===== STAGE 2: TRADE ENTRY =====
   if(StatsFilterEnabled)
   {
      double wr=0; int ns=0;
      if(!LookupSetupStats(key, wr, ns)) return;        // unknown setup -> skip
      if(ns < MinSamples || wr < MinWinRate) return;     // below win-rate gate -> skip
   }
   if(IsOverextended()) return;   // Ch.17 overextension regime filter (default OFF; gold/silver only)
   double szMult = LookupSizing(key);                      // 1.0 if table off; 0 = negative/unknown setup
   if(UseSizingTable && szMult <= 0) return;               // sizing table is the gate for negatives
   g_clusterSizeMult = (szMult > 0) ? szMult : 1.0;
   int maxB = (MaxConcurrentBaskets < 1) ? 1 : MaxConcurrentBaskets;
   if(CountOpenBaskets() >= maxB) return;                  // basket-concurrency cap (default 1 = one cluster at a time, unchanged)
