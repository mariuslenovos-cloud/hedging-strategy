//+------------------------------------------------------------------+
//| GENERATED FILE -- LOCKED FIBC config. Do NOT edit by hand.       
...
#define BUILD "FIBC-2026-07-13-XC4015-LOCKED"

//--- sizing
const double LotSize            = 0.02;  // LOCKED (was input)
const bool UseFibonacci       = true;     // 1,1,2,3,5,8.. per direction (else martingale x LotMultiplier)  // LOCKED (was input)
const int LotMultiplier      = 2;  // LOCKED (was input)
const bool UseCompounding     = true;  // LOCKED (was input)
const double CompoundingBase    = 3000.0;   // base lot scales with floor(balance/this)  // LOCKED (was input)
const int MaxSameTrades      = 5;        // max ladder depth per direction  // LOCKED (was input)
const int MaxFibMult         = 0;        // GENTLER LADDER: cap per-leg fib multiplier (0=uncapped 1,1,2,3,5,8..; e.g. 3 -> 1,1,2,3,3,3 = smaller deep legs = smaller floating basket = smaller floor loss, lower PF in chop. The DD dial that works regardless of where legs were placed.)  // LOCKED (was input)
const int MaxCompoundScale   = 3;        // [curb-give-back] cap the compounding multiplier floor(bal/CompoundingBase). 0=uncapped (current). e.g. 3 = base lot never grows past 3x -> baskets stay SMALL relative to a grown account -> the recovery keeps the rescue headroom it has at $3k -> fewer/smaller floor-hits. Trade-off: less compounding upside.  // LOCKED (was input)
//--- RECOVERY SIZING MODE (zone-recovery "break-even guarantee" math vs Fibonacci). A/B lever.
const int RecoverySizeMode   = 1;        // 0 = Fibonacci (default, UNCHANGED). 1 = BREAK-EVEN formula: each recovery leg = (RR+1)/RR x (opposite-side lots - same-side lots) x CostBuffer = the MINIMAL leg that makes the basket net-positive at the next favourable move, cost included. Leaner/more principled than Fibonacci -> smaller per-basket tail. (Derived from zone-recovery math; in a one-sided trend it floors to base = uniform lots.)  // LOCKED (was input)
const double RecoveryRR         = 3.0;      // mode 1 R:R (higher = gentler leg growth)  // LOCKED (was input)
const double RecoveryCostBuffer = 1.1;      // mode 1: size up x this to clear spread+commission+swap (1.1 = +10%)  // LOCKED (was input)
//--- entry signal (Goldminer embedded + trend gate)
const int grisk              = 7;        // fib C value (drives auto period/bands)  // LOCKED (was input)
const int GM_Period          = 0;        // 0 = auto (grisk*2+3)  // LOCKED (was input)
const double GM_UpperBand        = 0;       // 0 = auto (grisk+67)  // LOCKED (was input)
const double GM_LowerBand        = 0;       // 0 = auto (33-grisk)  // LOCKED (was input)
const double GM_GapMult          = 2.0;  // LOCKED (was input)
const int GM_GapMode          = 0;  // LOCKED (was input)
const double GM_FastMult         = 4.6;  // LOCKED (was input)
...
const int tradesperbar        = 1;  // LOCKED (was input)
//--- session
...
//--- exits
const bool UseBasketTrail       = true;  // LOCKED (was input)
const double MinFloatToActivate   = 40.0;  // LOCKED (was input)
const double BasketTrailAmount     = 15.0;  // LOCKED (was input)
const bool UseBuySellProfitThreshold = true;  // same-dir quick harvest  // LOCKED (was input)
const double BuySellProfitThreshold     = 50.0; // in POINTS*point (gold ~ $0.50) -- faithful to live  // LOCKED (was input)
const double BuySellProfitThresholdInCurrency = 0; // >0 overrides to a $ threshold  // LOCKED (was input)
//=== RISK-SAFE levers (DEFAULT OFF -> base == live; we A/B these on) ===
const bool UseBasketStop        = true;  // catastrophe floor: close ALL if book float <= -BasketMaxLossPct% balance  // LOCKED (was input)
const double BasketMaxLossPct     = 20.0;  // LOCKED (was input)
const bool UseDailyTrendFilter  = true;  // only enter WITH the D1 trend (cuts counter-trend tax)  // LOCKED (was input)
const int DailyMA_Period       = 50;  // LOCKED (was input)
const bool UseImbalanceLock     = true;  // stop adding to a side that outnumbers the other by >= ImbalanceThreshold AND is in loss  // LOCKED (was input)
const int ImbalanceThreshold   = 2;  // LOCKED (was input)
const bool UseMaxBasketLots     = false;  // cap TOTAL open lots -> bounds the Fibonacci martingale = caps the max basket loss (the DD tail)  // LOCKED (was input)
...
const bool UseVolRegimeFilter   = true;  // master switch  // LOCKED (was input)
const int VolATRFastPeriod     = 5;      // fast ATR(D1) = current regime  // LOCKED (was input)
const int VolATRSlowPeriod     = 60;     // slow ATR(D1) = baseline regime  // LOCKED (was input)
const double VolRegimeMult         = 2.5;   // HOT when ATR_fast/ATR_slow >= this  // LOCKED (was input)
const bool VolBlockAllEntries    = true;  // (a) HOT: block ALL new entries (sit out the storm)  // LOCKED (was input)
const bool VolBlockStacking      = false; // (b) HOT: block only ADDS (allow 1st entry/dir)  // LOCKED (was input)
const bool VolScaledFloor        = false;  // (c) HOT: WIDEN the catastrophe floor so baskets ride to recovery  // LOCKED (was input)
const double VolFloorMaxMult        = 2.0;  // (c) cap: floor% widens up to this x BasketMaxLossPct (30%->60%)  // LOCKED (was input)
...
const bool UseOverextFilter     = false;   // LOCKED (was input)
const double OverextATRMult        = 4.5;   // HOT when |close - D1EMA(DailyMA_Period)| / ATR(D1,VolATRSlowPeriod) >= this (gold ~90th pctile)  // LOCKED (was input)
const bool OverextBlockAdds      = true;  // true: block only ADDS when stretched (cap the deep fib legs, keep base flow); false: block ALL entries  // LOCKED (was input)
...
const string EquityLogFile        = "fibc_equity.csv";  // LOCKED (was input)
...
const bool UseFirstEntryGate    = true;  // LOCKED (was input)
const string FirstGateStatsCSV    = "gridstat_setups_gold.csv";  // GridStat shadow DB (Common\Files); same fingerprint  // LOCKED (was input)
const double FirstGateMinWinRate  = 0.55;   // win-rate threshold for BUY starts  // LOCKED (was input)
const double FirstGateSellMinWinRate = 0.65; // SELL-only threshold (0 = use FirstGateMinWinRate). Gold longs >> shorts -> set this higher (e.g. 0.65) to cut weak shorts.  // LOCKED (was input)
const int FirstGateMinSamples  = 10;  // LOCKED (was input)
...
const bool UseRecoveryHedge     = true;  // LOCKED (was input)
const double HedgeTriggerLoss     = 0.0;  // arm when book float <= -this ($)  // LOCKED (was input)
const double HedgeTriggerPctBal   = 3.0;    // >0: arm when book float <= -this%% of balance (overrides HedgeTriggerLoss; deposit-portable). Validated 3.  // LOCKED (was input)
const double RecoveryTargetUSD    = 40.0;   // close the WHOLE book once it recovers to +this ($)  // LOCKED (was input)
const bool UseRecoveryTrail     = true;  // [opt1] once recovered to +target, TRAIL the winner instead of flat-closing (+4%% on gold)  // LOCKED (was input)
const double RecoveryTrailGiveback = 20.0;  // give-back ($) from the recovery peak that closes (locks >= effective target)  // LOCKED (was input)
const double RecoveryTargetPct    = 10.0;    // [opt2] >0: scale target to this %% of the DEEPEST loss rescued (max w/ RecoveryTargetUSD). +3%% on gold at 10.  // LOCKED (was input)
const bool RecoveryRespectBreakFilters = false; // [curb] while rescuing, DON'T add a recovery leg when vol-regime HOT or price OVEREXTENDED (structural-break filters) -> stops piling into a stretched move about to whipsaw (the floor-hit cause). Needs UseVolRegimeFilter/UseOverextFilter on.  // LOCKED (was input)
const bool RecoveryScaleByLots  = false; // [curb-size] scale the recovery $ levers (RecoveryTargetUSD/RecoveryTrailGiveback/HedgeTriggerLoss) by the compounding lotScale=floor(bal/CompoundingBase) so they TRACK lot size as the account grows (the basket-trail already does this). Fixes the floor-hits that appear only on grown accounts.  // LOCKED (was input)
//--- misc
const string CommentText          = "FIB C";  // LOCKED (was input)
const int MagicSeed            = 0;  // LOCKED (was input)
...
int OnInit()
{
   ...
   {
      Print("LOCKED FIBC EA attached to WRONG symbol: ", _Symbol, " (accepts: GOLD/XAUUSD). ABORTING.");
      return(INIT_FAILED);
   }
   Print("LOCKED FIBC EA -- all inputs hard-coded (generated 2026-07-12); only StagedFloor levers visible");
