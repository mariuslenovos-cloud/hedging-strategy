//+------------------------------------------------------------------+
//| GENERATED FILE -- LOCKED OIL config. Do NOT edit by hand.       
...
#define EA_BUILD_VERSION "2026-07-12-P-MT4CONC-OIL-LOCKED"
...
const bool StatsFilterEnabled    = false;     // TRADE mode: filter by historical win rate. SET TO TRUE FOR TRADE MODE.  // LOCKED (was input)
const double MinWinRate            = 0.50;     // Legacy win-rate gate (used only if UseSizingTable=false)  // LOCKED (was input)
const int MinSamples            = 10;  // LOCKED (was input)
const string StatsCSVFile          = "gridstat_setups_oil_mt5.csv";  // LOCKED (was input)

//--- Expected-R sizing table (AFML rigor: sizes bets, replaces win-rate gate)
const bool UseSizingTable        = true;     // size by expected-R table; skip only negative-expectancy setups  // LOCKED (was input)
...
const bool UseRiskNormalizedLots = true;    // size each trade so 1R = RiskPctPerTrade% of balance (equalizes $ risk across symbols)  // LOCKED (was input)
const double RiskPctPerTrade       = 2.5;      // target risk per trade as % of balance (gold-equivalent ~4%)  // LOCKED (was input)
...
const double BasketMaxLossPct      = 10.0;     // % of account balance — hard per-basket loss cap  // LOCKED (was input)
const int MaxConcurrentBaskets  = 3;        // [MT4-CONC] 1 = single basket (unchanged). >1 = N concurrent baskets, each grid-managed + a portfolio floor (oil/trenders need 3).  // LOCKED (was input)
const bool UseEntrySL            = false;     // [MT4-CONC] true = per-entry hard SL (unchanged). false = no per-entry stop, rely on the basket floor (oil pulls back -> SL-off lets the grid recover).  // LOCKED (was input)
...
int OnInit() {
   ...
   if(_Symbol != "OILCash" && _Symbol != "SpotCrude")
   {
      Print("LOCKED OIL EA attached to WRONG symbol: ", _Symbol, " (accepts: OILCash/SpotCrude). ABORTING.");
      return(INIT_FAILED);
   }
   Print("LOCKED OIL EA -- all inputs hard-coded (generated 2026-07-12); only StagedFloor levers visible");
